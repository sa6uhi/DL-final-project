...
Email: nadirr2609@gmail.com, sabuhi.nazarov11@gmail.com, habil.huseynov25@aiacademy.edu.az
}
}
\maketitle
\begin{abstract}
...
DAE scorer (0.71 ms eager P99 at batch size 1). The system additionally provides SHAP-based anomaly explanations,
...
with PyTorch CPU builds; the data split is strictly
chronological (70/15/15 by \texttt{TransactionDT}) with scalers and vocabularies
fitted on the training block only. Tests cover the gating network, conformal predictor,
...
\end{tabular}
\end{table}
The test PR-AUC of 0.3412 is numerically lower than the development-validation
PR-AUC mean of 0.4395 from the architecture ablation, indicating that
out-of-time generalization is harder than performance measured
...
significance test) indicate that the learned fusion model
...
DAE 0.42 ms, gate 0.12 ms) showed that FT-CAT dominates the neural
...
were measured on a single-CPU container host and are
hardware-dependent: absolute timings will vary across CPUs, while the relative
ordering of components and batch-size trends should reproduce.
...
files) representations. Export-time
parity checks compare serialized outputs with their PyTorch reference
implementations. The trained FT-CAT (94{,}305 parameters) and learned-gate
(225 parameters) checkpoints have been exported through the same pipeline to
\texttt{models/serialized/} (EXIR and ONNX); serialized parity reporting for
these two models follows the DAE procedure.
...
transfer size), dominated by the PyTorch
CPU runtime and model weights.
...
A fourth limitation was identified and corrected during development. An
early version of the automated test suite for the conformal evaluation
script wrote its output artifacts to the same path used for production
evaluation rather than an isolated temporary directory. Because tests were
run locally during development, this could silently regenerate the serving
threshold artifact from a small synthetic test fixture instead of the real
calibration split. The issue was caught, the test harness was corrected to
redirect all artifact writes to an isolated path, and the affected
artifacts were regenerated from the real calibration data. It is reported
here for transparency, and because it illustrates a general risk in ML
pipelines: test code that exercises production code paths without
isolating side effects can silently corrupt deployed artifacts without
raising any error.
A fifth limitation concerns model complexity versus measured benefit. An
ablation comparing the fused FT-CAT transformer (94{,}305 parameters)
against a self-attention-only variant and a flat MLP control, trained
under an identical protocol across three random seeds, found the flat MLP
control achieved the highest mean test PR-AUC (0.3698) of the three,
exceeding both the self-attention-only variant (0.3376) and the full
FT-CAT model (0.3380). Capacity and training budget were held fixed rather
than independently tuned per variant, so this does not prove categorical
feature interactions are unhelpful on this dataset, but it means the
transformer's added architectural complexity is not currently justified by
test-set performance, and is reported rather than left implicit. A related
design choice worth flagging is the use of card1 (and addr1) as the entity
key for constructing transaction-history features. card1 is a
semi-persistent per-card identifier rather than a true anonymized session
ID, so history-based features may partially encode card identity rather
than purely behavioral signal.
...
peak tested throughput of approximately 2,746 transactions per
second.
