...
Email: nadir.eskerov25@aiacademy.az, aqsin.fataliyev25@aiacademy.az,\\
habil.huseynov25@aiacademy.edu.az, sebuhi.nezerov25@aiacademy.az
}
}
...
held-out test set, the FT-CAT model achieved a PR-AUC of 0.3412 and ROC-AUC of
0.8144. At a target miscoverage level of 0.01, the conformal system achieved
...
(chronological test split; temporal shift violates strict exchangeability, so
this is a measured result, not a production guarantee).
CPU benchmarking measured a median batch-1 neural inference latency of 6.56 ms
(P99 26.95 ms) and peak tested throughput of approximately 2,746 transactions
per second at batch size 128; the 15 ms P99 service objective is met by the
DAE scorer (0.71 ms eager P99 at batch size 1). The system additionally provides
SHAP-based anomaly explanations, subgroup operational robustness auditing,
...
tests, CPU inference benchmarking, and retrospective subgroup robustness
analysis.
...
\begin{itemize}
    ...
    model serialization, API serving, and an analyst-facing dashboard; and
    \item retrospective subgroup analysis for identifying variation in review
    workload and detection performance across available transaction attributes.
\end{itemize}
...
curve (PR-AUC), ROC-AUC, and true-positive rate at the 1\% false-positive-rate
operating point (\texttt{max\_fpr: 0.01}).
...
Brier score (mean squared probability error), and expected calibration error
(ECE, binned confidence-accuracy gap).

Deployment evaluation reports latency percentiles and throughput over several
batch sizes.

For retrospective subgroup analysis, we report transaction count, observed
fraud rate, review rate, recall, false-positive rate, and PR-AUC for available
transaction and platform attributes. Subgroups with fewer than 30 transactions
or fewer than 10 observed fraud cases are flagged as unreliable for
subgroup-level interpretation.
...
used for reproducibility (\texttt{seed\_everything(42)} for training runs;
gate robustness evaluated at seeds 42, 123, and 2026; FT-CAT checkpoint records
seed 42). Benchmarks were measured on a single-CPU container host
with PyTorch CPU builds; the data split is strictly chronological
(70/15/15 by \texttt{TransactionDT}) with scalers and vocabularies fitted on
the training block only.

Automated regression tests cover the gating network, conformal predictor,
evaluation utilities, SHAP integration, dashboard behavior, serialization, and
...
Logistic regression, random forest, and LightGBM baselines were trained on
the upstream-model training split and evaluated on the chronological
development-validation split (88{,}581 transactions).
Table~\ref{tab:baselines} reports validation PR-AUC and ROC-AUC.

\begin{table}[t]
\caption{Classical baselines on the chronological development-validation set.}
\label{tab:baselines}
\centering
\begin{tabular}{@{}lcc@{}}
\toprule
...
\midrule
Logistic Regression (balanced) & 0.4471 & 0.8609 \\
Random Forest (200 trees, depth 16) & 0.4698 & 0.8819 \\
LightGBM (weighted) & 0.5207 & 0.9088 \\
\bottomrule
\end{tabular}
\end{table}

The random forest uses a maximum depth of 16 and a minimum leaf size of 10:
without a depth limit, 200 fully grown trees on the full training data
produced a 550\,MB checkpoint with multi-second inference stalls, while the
constrained model compresses to 57\,MB and scores the full validation split
in 0.3\,s on CPU. Final out-of-time test comparison against the deep models
uses the same chronological held-out test set.

\subsection{FT-CAT Performance}

Table~\ref{tab:ftcat} reports the recorded final FT-CAT performance on the
...
The recorded test PR-AUC of 0.3412 is numerically lower than the
development-validation PR-AUC mean of 0.4395 from the architecture ablation,
indicating that out-of-time generalization is harder than performance measured
within the development period. This reinforces the importance of chronological
evaluation for fraud detection.

\subsection{Focal-Loss Sensitivity}

A controlled focal-loss sensitivity experiment evaluated five focusing
parameters,
$\gamma \in \{0.5,1.0,2.0,3.0,5.0\}$, crossed with three class-balancing
parameters,
$\alpha \in \{0.1,0.25,0.5\}$, using seed 42 and an eight-epoch training
budget. A weighted binary-cross-entropy configuration was included as a
control.

To avoid test-set leakage in hyperparameter selection, configurations are
compared using their best development-validation PR-AUC rather than their
held-out test PR-AUC. Under this criterion, the strongest evaluated focal-loss
cell was $\gamma=0.5,\alpha=0.5$, with validation PR-AUC 0.4325. Its held-out
test PR-AUC was 0.3282. The weighted-BCE control achieved validation PR-AUC
0.4076 and test PR-AUC 0.3264.

The sweep indicates that focal-loss behavior is sensitive to both $\gamma$ and
$\alpha$: increasingly aggressive focusing does not monotonically improve
performance. In particular, large $\gamma$ values can substantially reduce
PR-AUC under the fixed training budget. Because the sweep uses one random seed
and an eight-epoch budget, it should be interpreted as a sensitivity analysis
rather than a definitive hyperparameter comparison.

\subsection{Attention Analysis}

To inspect how the FT-CAT checkpoint uses historical context, attention weights
were summarized over 1,024 sampled transactions, balanced between 512 fraud and
512 legitimate examples. The analysis covers four attention heads and a
five-position historical sequence.

Across the four heads, the most recent historical position receives substantial
attention for both classes. Mean attention to the most recent slot is
approximately 0.2875 for fraud examples and 0.3097 for legitimate examples.
Several missingness-indicator features, including \texttt{V261\_is\_nan},
\texttt{V256\_is\_nan}, and \texttt{V215\_is\_nan}, appear among the features
with the strongest observed history attention.

These results provide a descriptive view of how the trained network allocates
attention across available historical context. Attention weights should not be
interpreted as causal feature importance or as a complete explanation of the
FT-CAT prediction.
...
SD (sample, $n=3$) & 0.0077 & 0.0032 & 0.0070 \\
\bottomrule
\end{tabular}
\end{table}

The relatively small sample standard deviations (descriptive, $n=3$; no
significance test) indicate that the learned fusion model is not strongly
dependent on a single random initialization. Seed 42 was used for the production
checkpoint according to the project's predetermined random seed rather than
selected post hoc from the three results.
...
\includegraphics[
    width=\columnwidth,
    keepaspectratio
]{../figures/hybrid\_gating/seed\_robustness.png}
\caption{Learned-gate performance across the three evaluated random seeds.}
\label{fig:gate_robustness}
\end{figure}
...
Figure~\ref{fig:conformal_workload} illustrates the operational trade-off
between predictive coverage and analyst workload across the evaluated conformal
operating points. At the selected $\alpha=0.01$ operating point, the system
achieves 98.99\% empirical marginal coverage while routing 23.19\% of test
...
\includegraphics[
    width=\columnwidth,
    keepaspectratio
]{../figures/conformal/coverage\_vs\_workload.png}
\caption{Empirical conformal coverage and human-review workload across evaluated
miscoverage levels.}
\label{fig:conformal_workload}
\end{figure}

Figure~\ref{fig:threshold_sensitivity} provides a complementary view of the
sensitivity of the conformal triage policy across evaluated operating points.
The analysis emphasizes that the calibrated operating point affects both
statistical coverage and downstream analyst workload; operational thresholds
should therefore be chosen jointly with review-capacity requirements rather
than from predictive performance alone.
...
\includegraphics[
    width=\columnwidth,
    keepaspectratio
]{../figures/conformal/threshold\_sensitivity.png}
\caption{Sensitivity of conformal triage behavior across evaluated operating
points.}
...
silhouette score was 0.1046 (training log).
...
\includegraphics[
    ...
    keepaspectratio
]{../figures/autoencoder\_latent/tsne\_latent\_space.png}
...
global samples, the mean Spearman correlation of feature rankings was 0.8980,
while mean top-feature Jaccard similarity was 0.4809.
...
\includegraphics[
    width=\columnwidth,
    keepaspectratio
]{../figures/explainability/dae\_shap\_global\_importance.png}
\caption{Global SHAP feature importance for the DAE reconstruction-anomaly
component.}
\label{fig:shap_global}
\end{figure}

The consistency analysis in Fig.~\ref{fig:shap_consistency} complements the
global importance plot by measuring how stable the DAE feature-attribution
ranking is across independent samples. The high rank correlation indicates
broad stability, while the more moderate Jaccard similarity demonstrates that
the exact membership of the highest-ranked feature subset varies across
samples.
...
\includegraphics[
    width=\columnwidth,
    keepaspectratio
]{../figures/explainability/dae\_shap\_consistency.png}
\caption{Consistency of DAE SHAP feature attributions across independent
samples.}
\label{fig:shap_consistency}
\end{figure}

Figure~\ref{fig:shap_waterfall} provides a local example of the DAE explanation
workflow. The waterfall visualization decomposes the anomaly-model output for
an individual transaction into feature-level contributions. As with the global
analysis, the visualization explains the DAE anomaly component only and should
not be interpreted as a decomposition of the final learned-gate decision.
...
\includegraphics[
    width=\columnwidth,
    keepaspectratio
]{../figures/explainability/dae\_shap\_waterfall.png}
\caption{Local SHAP waterfall explanation for a selected DAE anomaly example.}
\label{fig:shap_waterfall}
\end{figure}

\subsection{Subgroup Operational Robustness Audit}

As a retrospective robustness check, the final triage behavior was evaluated
across available transaction subgroups defined by \texttt{DeviceType},
\texttt{card4}, \texttt{card6}, \texttt{ProductCD}, and
\texttt{P\_emaildomain}. For each subgroup, the audit reports transaction count,
observed fraud rate, review rate, recall, false-positive rate, and PR-AUC.
Groups with fewer than 30 transactions or fewer than 10 observed fraud cases
are flagged as unreliable for subgroup-level interpretation.

The audit reveals substantial heterogeneity in operational behavior. For
example, among the reported \texttt{ProductCD} groups,
\texttt{ProductCD=W} has an approximately 14.2\% review rate and 43\% recall,
whereas \texttt{ProductCD=S} has an approximately 89.6\% review rate and 97\%
recall. Variation is also observed across \texttt{DeviceType},
\texttt{card4}, and \texttt{card6} groups. These differences demonstrate that
aggregate coverage and review-rate statistics do not fully characterize how
the triage policy behaves across transaction subpopulations.

These attributes describe transaction, payment, product, device, and platform
characteristics rather than protected demographic characteristics.
Accordingly, this analysis is an operational subgroup robustness audit and
should not be interpreted as a demographic fairness assessment. It is intended
to identify heterogeneous model behavior that warrants monitoring before
...
6.56 ms. Component-level profiling at batch size 1 (median: FT-CAT 5.05 ms,
DAE 0.42 ms, gate 0.12 ms) showed that FT-CAT dominates the neural compute
path, whereas learned-gate inference itself is lightweight.
...
interpreted as complete HTTP API latency. All latency and throughput figures
were measured on a single-CPU container host and are hardware-dependent:
absolute timings will vary across CPUs, while the relative ordering of
components and batch-size trends should reproduce.

At the component level, the servable DAE scorer was benchmarked separately
with 200 timed iterations per cell on CPU across batch sizes 1--64. At batch
size 1, P99 latency was 0.71\,ms for PyTorch eager execution, 0.99\,ms for the
EXIR export, and 0.14\,ms for ONNX, all well within the 15\,ms service
objective; DAE-scorer P99 remained below 1\,ms at every tested batch size.
...
\includegraphics[
    width=\columnwidth,
    keepaspectratio
]{../figures/inference\_benchmark/inference\_throughput.png}
...
The trained DAE (485{,}440 trainable parameters plus 513 non-trainable
batch-normalization buffers, 1.95\,MB checkpoint) was serialized to
both PyTorch EXIR (2.00\,MB) and ONNX (1.96\,MB across the model and data
files) representations. Export-time parity checks compare serialized outputs
with their PyTorch reference implementations.

The trained FT-CAT (94{,}305 parameters) and learned-gate (225 parameters)
checkpoints have been exported through the same pipeline to
\texttt{models/serialized/} in EXIR and ONNX formats. Serialized parity
reporting for these models follows the DAE procedure.

The containerized FastAPI scoring service (single CPU worker) ships as an
approximately 2\,GB uncompressed inference image (roughly 450\,MB compressed
transfer size), dominated by the PyTorch CPU runtime and model weights.

The product additionally exposes a FastAPI scoring interface and a Streamlit
analyst dashboard. The dashboard surfaces live prediction, batch analysis,
model insights, conformal triage, explainability, and artifact-readiness
information.
...
The subgroup audit further demonstrates that aggregate performance can conceal
substantial operational heterogeneity. Review workload and fraud recall differ
considerably across some product, device, and card-related subgroups. These
differences do not establish demographic unfairness because the audited
variables are not protected demographic attributes, but they show that a
single aggregate operating point can affect transaction subpopulations very
differently. Production deployment would therefore require continued subgroup
monitoring and investigation of the mechanisms producing these disparities.
...
A fourth limitation was identified and corrected during development. An early
version of the automated test suite for the conformal evaluation script wrote
its output artifacts to the same path used for production evaluation rather
than an isolated temporary directory. Because tests were run locally during
development, this could silently regenerate the serving threshold artifact from
a small synthetic test fixture instead of the real calibration split.

The issue was caught, the test harness was corrected to redirect all artifact
writes to an isolated path, and the affected artifacts were regenerated from
the real calibration data. It is reported here for transparency, and because it
illustrates a general risk in ML pipelines: test code that exercises production
code paths without isolating side effects can silently corrupt deployed
artifacts without raising any error.

A fifth limitation concerns model complexity versus measured benefit. An
ablation comparing the fused FT-CAT Transformer (94{,}305 parameters) against
a self-attention-only variant and a flat MLP control, trained under an identical
protocol across three random seeds, found that the flat MLP control achieved
the highest mean test PR-AUC (0.3698) of the three, exceeding both the
self-attention-only variant (0.3376) and the full FT-CAT model (0.3380).
Capacity and training budget were held fixed rather than independently tuned per
variant, so this does not prove categorical feature interactions are unhelpful
on this dataset. It does mean that the transformer's added architectural
complexity is not currently justified by test-set performance and is reported
rather than left implicit.

A related design limitation is the construction of transaction histories using
\texttt{card1} together with \texttt{addr1} as a pseudo-entity key. These are
masked dataset attributes rather than a verified persistent customer, account,
or card identifier. Consequently, the resulting sequences may combine
transactions that do not belong to a single real-world entity. History-based
features should therefore be interpreted as pseudo-entity behavioral context
rather than true per-card transaction histories.

The focal-loss sensitivity experiment is also intentionally limited. It uses a
single random seed and a fixed eight-epoch budget, so the observed differences
across $\gamma$ and $\alpha$ should not be interpreted as a fully tuned
comparison. The experiment is used to characterize sensitivity and to enforce
validation-based rather than test-based configuration selection.

The attention analysis is descriptive rather than causal. Attention weights
show where the trained FT-CAT allocates representation weight across historical
positions but do not establish that highly attended features caused the model's
prediction.

The SHAP implementation is similarly scoped. Current explanations describe the
DAE anomaly component and do not provide a complete attribution of the final
learned-gate decision. Extending explainability to the complete fused system is
...
The final evaluation additionally examines focal-loss sensitivity, attention
behavior, learned-gate seed robustness, DAE latent structure, SHAP attribution
consistency, and subgroup operational heterogeneity. Together, these analyses
provide a broader view of model robustness and limitations than aggregate
...
6.56 ms (P99 26.95 ms; the 15 ms P99 objective is met by the DAE scorer) and
peak tested throughput of approximately 2,746 transactions per second.

Future work should investigate class-conditional conformal calibration,
distribution-shift and subgroup monitoring, fused-model explanations,
alternative sequence-identity definitions, and operating points that support
safe automatic blocking.
