...
def main() -> None:
    ...
    train_df = pd.read_parquet(processed_dir / "train.parquet").sample(
        frac=SAMPLE_FRAC, random_state=42
    )
    val_df = pd.read_parquet(processed_dir / "val.parquet").sample(
        frac=SAMPLE_FRAC, random_state=42
    )
    ...
    for k in k_values:
        ...
        def flatten_sequences(df: pd.DataFrame) -> pd.DataFrame:
            ...
            seq_df = pd.DataFrame(
                seq_arrays.reshape(len(df), -1), columns=flattened_cols, index=df.index
            )
        ...
        lgbm = LGBMClassifier(
            n_estimators=50, scale_pos_weight=scale_weight, verbose=-1, random_state=42
        )
