...
def calculate_financial_cost(
    ...
    fn_cost=np.sum(amount[false_negatives])
    ...
    results = {
        ...
        "savings_percentage": (total_savings / total_fraud_loss * 100) if total_fraud_loss > 0 else 0.0
    }

    logger.info(f"Financial Evaluation -> Model Cost: ${total_model_cost:,.2f} | Net Savings: ${total_savings:,.2f} ({results['savings_percentage']:.1f}%)")
    return results
